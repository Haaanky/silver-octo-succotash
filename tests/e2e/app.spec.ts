...
test.describe('Inloggningssida', () => {
  ...
  test('visar felmeddelande vid fel lösenord', async ({ page }) => {
    ...
    await expect(page.getByText('Felaktigt e-post eller lösenord')).toBeVisible();
  });
...
test.describe('Lagerlista', () => {
  ...
  test('visar tabellrubriker', async ({ page }) => {
    ...
    await expect(page.locator('table thead th').filter({ hasText: 'Lager' })).toBeVisible();
  });
  ...
  test('sökfältet filtrerar produkter', async ({ page }) => {
    ...
    await expect(page.getByText('Inga produkter matchar sökningen', { exact: false })).toBeVisible();
...
test.describe('Navigation', () => {
  ...
  test('navbar visas efter inloggning', async ({ page }) => {
    ...
    await expect(page.getByText('LagerApp').first()).toBeVisible();
  });
...
test.describe('Produkthantering', () => {
  ...
  test('kan skapa ny produkt', async ({ page }) => {
    ...
    const inputs = page.locator('.card input[type="text"], .card input:not([type])');
    ...
    await expect(page.locator('h2').filter({ hasText: /Ny produkt|Redigera/ })).toHaveCount(0, { timeout: 10_000 });
  ...
  test('kan redigera en produkt', async ({ page }) => {
    ...
    const nameInput = page.locator('.card input[type="text"], .card input:not([type])').first();
    const editedName = `Redigerad ${Date.now()}`;
    await nameInput.fill(editedName);
    await page.click('button:has-text("Spara")');

    await expect(page.locator('h2').filter({ hasText: /Redigera/ })).toHaveCount(0, { timeout: 10_000 });
...
test.describe('Skanna / Transaktioner', () => {
  ...
  test('visar varning för okänd streckkod', async ({ page }) => {
    ...
    await expect(page.getByText('Ingen produkt hittades.', { exact: false })).toBeVisible({ timeout: 15_000 });
  });
  ...
  test('kan registrera inleverans för befintlig produkt', async ({ page }) => {
    ...
    await expect(page.locator('table tbody tr').first()).toBeVisible({ timeout: 15_000 });
    // Barcode is in the 3rd column (index 2): Namn, SKU, Streckkod
    ...
    await expect(page.getByText(/inleverans/i)).toBeVisible({ timeout: 15_000 });
  });
});
...
test.describe('Export', () => {
  ...
  test('visar exportknappar', async ({ page }) => {
